...
sourceDir=os.path.split(os.path.abspath(sys.argv[0]))[0]#源代码目录
...
def vocoder(input:List[str]):
    ...
    singerDir:str = plugin.voicedir
    singer=singerManager.getsinger(singerDir)
    ...
    #将歌手路径以命令行参数的形式传入合成器
    #目前config从歌手路径加载，模型从源代码文件夹加载
    dsConfig=singer.dsConfig
    exp_name=dsConfig["exp_name"]
    sys.argv = [
        os.path.join(sourceDir,"inference/svs/ds_e2e.py"),
        '--config',
        os.path.join(singerDir,dsConfig["config"]),
        '--exp_name',
        exp_name
    ]
    #合成
